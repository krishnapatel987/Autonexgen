...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentView }) => {
  ...
  const handleMobileNav = (view: AppView, sectionId?: string) => {
    setIsMenuOpen(false);
    onNavigate(view, sectionId);
  };

  // Shared button style for consistent interaction
  const navButtonClass = (isActive: boolean, isReviews: boolean) => `
    px-2.5 lg:px-4 py-1.5 text-[10.5px] lg:text-[11.5px] font-medium rounded-full 
    transition-all duration-300 whitespace-nowrap outline-none
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full z-[60] flex items-center px-4 md:px-6 lg:px-8 py-2.5 md:py-3 border-b bg-white/[0.02] backdrop-blur-2xl border-white/[0.08] shadow-[0_4px_24px_0_rgba(0,0,0,0.6)]">
        <div className="flex-none flex justify-start items-center gap-2 md:gap-3">
          <button 
            ...
            className="md:hidden flex items-center justify-center w-8 h-8 rounded-lg border border-white/10 bg-white/5 text-white transition-all hover:scale-110 active:scale-90"
          ...
          <button 
            ...
            className="group flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95 text-left"
        ...
        <div className="hidden md:flex flex-1 justify-center px-4">
          <div className="flex items-center gap-0.5 lg:gap-1 p-0.5 rounded-full border backdrop-blur-md bg-white/5 border-white/10">
            {['home', 'services', 'results', 'reviews', 'about', 'contact'].map((viewName) => (
              <button 
                key={viewName}
                onClick={() => onNavigate(viewName as AppView)}
                className={navButtonClass(currentView === viewName, viewName === 'reviews')}
              >
        ...
        <div className="flex-none flex justify-end items-center gap-2 md:gap-3 lg:gap-6">
          <a 
            ...
            className="hidden xl:flex items-center gap-2 text-[11px] font-medium transition-all duration-300 hover:scale-110 active:scale-95 text-blue-400 hover:text-blue-300 px-3 py-1.5 rounded-lg hover:bg-white/5"
          >
      ...
      <div className="md:hidden fixed bottom-4 right-4 z-[70] animate-in fade-in slide-in-from-bottom-4 duration-500">
        <button 
          onClick={() => onNavigate('contact')}
          className="flex items-center gap-1.5 px-3.5 py-2 bg-blue-600 text-white font-bold rounded-lg shadow-[0_8px_20px_rgba(37,99,235,0.25)] border border-blue-400/20 hover:scale-105 active:scale-95 transition-all hover:bg-blue-500"
        >
        ...
        <div className="flex flex-col h-full pt-32 px-6 pb-12 overflow-y-auto">
          <div className="flex flex-col gap-2">
            {[
              ...
              <button 
                key={item.label}
                onClick={() => handleMobileNav(item.view as AppView)}
                className={`w-full text-left py-4 px-6 text-xl font-medium rounded-2xl border transition-all hover:scale-[1.02] active:scale-95 ${
          ...
          <div className="mt-auto pt-12">
            <div className="p-6 rounded-3xl border border-white/5 bg-white/[0.02]">
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="flex items-center gap-4 text-white hover:text-blue-400 transition-all hover:scale-105 mb-6 py-2 rounded-xl hover:bg-white/5"
              >
              ...
              <button 
                ...
                className="w-full py-4 bg-blue-600 text-white font-bold rounded-xl flex items-center justify-center gap-2 active:scale-95 hover:scale-[1.02] hover:bg-blue-500 transition-all shadow-lg"
              >
                Book AI Audit
